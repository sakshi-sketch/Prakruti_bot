...
def askQuestion(msg: Reply, session: Session) -> dict | str:
    ...
    if start_questionaire:
        ...
        if limit <= i:
            ...
            msg = f"Your Prakruti is <b>{prakriti.prakriti}</b>"
            if prakriti.prakriti.lower().strip() == already_known and ask_Known:
                ...
                return [
                    msg,
                    {"share": "Prakruti", "dosha": prakriti.prakriti.title()},
                    ...
                    {
                        "question": "Want Diet & LifeStyle Recommendation based on Your Prakruti?",
                        "options": {0: "yes", 1: "no"},
                    },
                ]
            clearAll(exclude=True)
    ...
    try:
        if flag:
            ...
            if umessage == "yes" and not confirm:
                ...
                return {
                    "response": {
                        "question": "Do you already know your Prakruti? If Yes, could you please share it with me?",
                        "options": {0: "yes", 1: "no"},
                    }
                }
            elif not confirm and umessage == "no":
                clearAll()
                return {"response": "Skipped Predicting the Prakruti"}
            elif not confirm:
                ...
                if start_questionaire:
                    ...
                    if ask_Known:
                        ...
                        return {
                            "response": [
                                "Thank you for taking the time to share your Prakruti with me! Your input is invaluable and will greatly assist me in providing you with more personalized recommendations.",
                                {
                                    "question": "What is Your Prakruti",
                                    "options": {
                                        0: "Vata",
                                        1: "Pitta",
                                        2: "Kapha",
                        ...
                        if start_questionaire:
                            return {
                                "response": [
                                    "Answer the Above Set of Questions to Predict Your Prakruti",
                                    askQuestion(msg, session),
                                ]
                            }
                    else:
            ...
            if str(msg.get("message")).lower().strip() == "yes":
                ...
                diet.append(
                    "Thank You For predicting Prakruti with <b>PrakrutiBot</b>")
                diet.append(
                    "Your personalized advice on lifestyle, diet, and related videos for your Prakruti will be ready to download in PDF format soon. Please be patient while it processes.")
                clearAll()
                return {"response": diet}
            else:
                clearAll()
                return {"response": ["Skipped Diet Recommendation", "Thank You For predicting Prakruti with <b>PrakrutiBot</b>"]}
        else:
